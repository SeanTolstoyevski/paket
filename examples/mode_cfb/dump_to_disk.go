...
func main() {
	encFiles, err := paket.New(paket.Option{Key: []byte("djeiwj32ov919217xabejiwqpoapkS2032vmQ"),
		Salt:      PaketSalt,
		PaketFile: "./../../testdata/cfb.data",
		Table:     PaketData,
		Mode:      paket.MODECFB})
	if err != nil {
	...
	fmt.Println("Paket: Mode: CFB - write all data in table to disk")
	fileCount := 0
	fileBytesTotal := 0
	...
	fmt.Println("Paket: Mode: CFB - write to disk finished")
